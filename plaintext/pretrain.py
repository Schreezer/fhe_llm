...
def main_training_process(cfg, setup):
    ...
    cfg.arch.attention.is_train = True
    cfg.arch.is_train = True
    cfg.arch.norm = cfg.arch.train_norm
    cfg.arch.nonlin = cfg.arch.train_nonlin
    cfg.train.steps = cfg.arch.full_steps
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            log.info(f'Trainable parameters: {name}: {param.device}')
    for name, param in model.named_parameters():
        if not param.requires_grad:
            log.info(f'Untrainable parameters: {name}: {param.device}')
    total_trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad) # Total # of trainable params of The Model
    log.info(f'Total number of trainable parameters: {total_trainable_params}')
