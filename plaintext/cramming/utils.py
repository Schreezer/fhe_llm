...
def main_launcher(cfg, main_fn, job_name=""):
    print(colored('main_launcher', 'light_red'))
    ...
    cfg = pathfinder(cfg)
    # print(colored('cfg: {}'.format(cfg), 'blue'))
    ...
    # main_fn = main_training_process
    metrics = main_fn(cfg, setup)
    
    if metrics != "distillation":
...
def main_launcher_modified(cfg, main_fn, job_name=""):
    print(colored('main_launcher', 'light_red'))
    """This is boiler-plate code for a launcher."""
    launch_time = time.time()
    # Set definitive random seed:
