...
def group_parameters(model, cfg_train):
    # print(f'cfg_train: {cfg_train}')
    model_parameters = list(model.named_parameters())
    # print(f'model_parameters: {model_parameters}')
    # cfg_train.limited_decay_keys = []
    # log.info(f'Modified cfg_train.limited_decay_keys: {cfg_train.limited_decay_keys}, so weight decay to all parameters.')
...
def prepare_downstream_dataloader_modified(dataset, tokenizer, mode, cfg_impl):
    if mode == "training":
        if torch.distributed.is_initialized():
            sampler = torch.utils.data.distributed.DistributedSampler(
                dataset,
    ...
    # Implementation details for dataloaders:
    collate_fn = transformers.DataCollatorWithPadding(tokenizer, padding='max_length', max_length=128)
    os.environ["TRANSFORMERS_NO_ADVISORY_WARNINGS"] = "true"  # disable here because otherwise the collation generates a ton of errors
    # collate_fn = transformers.DefaultDataCollator()
    num_workers = get_num_workers(cfg_impl)
